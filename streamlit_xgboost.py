...
FORECAST_PATH = ROOT / "data" / "forecasts" / "load_forecast_xgb.parquet"
BACKTEST_FORECAST_PATH = ROOT / "data" / "forecasts" / "load_backtest_forecast_xgb.parquet"
METRICS_PATH = ROOT / "data" / "metrics" / "load_backtest_metrics_xgb_168h.txt"


st.set_page_config(
    page_title="XGBoost Load Forecast Operations",
    layout="wide",
)
...
def _parse_per_series_metrics(lines: list[str]) -> pd.DataFrame:
    # XGBoost reports do not emit AIC/BIC/HQIC, so those trailing columns are optional.
    pattern = re.compile(
        r"^\s*(?P<region>\S+)\s+"
        r"(?P<series>.+?)\s+"
        r"(?P<n>\d+)\s+"
        ...
        r"(?P<mape>[-+]?\d*\.?\d+)\s+"
        r"(?P<smape>[-+]?\d*\.?\d+)"
        r"(?:\s+(?P<aic>[-+]?\d*\.?\d+)\s+(?P<bic>[-+]?\d*\.?\d+)\s+(?P<hqic>[-+]?\d*\.?\d+))?"
        r"\s*$"
    )
    ...
    if not rows:
        return pd.DataFrame(columns=["region", "series", "n", "mae", "rmse", "mape", "smape"])
...
def show_missing_data_help(actuals: pd.DataFrame, forecasts: pd.DataFrame) -> None:
    ...
    st.info(
        "XGBoost dashboard data is missing. Run `python -m src.jobs.run_ingest` and then "
        "`python -m src.jobs.run_train_and_forecast` with `FORECAST_MODEL=xgboost` set in your "
        "environment to generate these outputs."
    )
...
st.title("XGBoost Load Forecast Operations")
st.caption("Operational dashboard for EIA regional load actuals, XGBoost forecasts, and recent backtest performance.")
...
if selected_region:
    ...
    if backtest_forecasts.empty:
        st.info("No backtest forecast file found. Run the XGBoost training job to generate it.")
    else:
